...
Output: /drawings page, 3 new components (DrawingTabs, ExportMenu, DrawingViewer), 2 new hooks (useSvgExport, useDrawingStyle), sidebar update.
</objective>
...
<task type="auto">
  <name>Task 2: Create /drawings page and add sidebar navigation link</name>
  <files>
    frontend/src/app/drawings/page.tsx
    frontend/src/components/layout/Sidebar.tsx
  </files>
...
1. Dedicated /drawings page -- implemented in this task
2. Embedded "Drawings" tab within cartridge/rifle detail pages -- DEFERRED (these detail pages don't exist yet as standalone pages; the current CRUD pages are table-based with inline edit, not detail views)
3. Linked from simulation results -- The assembly drawing already accepts simulation prop. When simulation results pages want to link to drawings, they can navigate to /drawings?cartridge_id=X&rifle_id=Y&tab=assembly and pass simulation data via URL params or a shared context. This wiring can be added incrementally. For now, the /drawings page is the primary entry point.

We implement entry point 1 fully. Entry points 2 and 3 are structurally supported (DrawingViewer is a reusable component that can be embedded anywhere) but the actual page integrations are deferred to when detail pages exist.
  </action>
  <verify>
    <automated>cd frontend && npx tsc --noEmit 2>&1 | head -30</automated>
  </verify>
  <done>/drawings page renders with cartridge/rifle/bullet selectors and DrawingViewer, sidebar has "Dibujos Tecnicos" link, all compiles cleanly</done>
</task>
