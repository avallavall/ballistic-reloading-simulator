...
must_haves:
  truths:
    ...
    - "The Rifle create/edit form accepts three optional nullable chamber fields (freebore_mm, throat_angle_deg, headspace_mm) with physical limit validation"
...
Output: Rifle model with 3 new nullable fields + Alembic migration + 8 frontend library files in `lib/drawings/` (split across Tasks 2 and 3) + jsPDF/svg2pdf.js installed.
</objective>
...
<task type="auto">
  <name>Task 2: Create drawing type definitions, themes, hatching patterns, and export utilities + install npm dependencies</name>
  <files>
    frontend/package.json
    frontend/src/lib/drawings/types.ts
    frontend/src/lib/drawings/themes.ts
...
5. **frontend/src/lib/drawings/export.ts** -- Export utilities:
  ...
  <done>4 files exist in frontend/src/lib/drawings/ (types, themes, hatching-patterns, export), jsPDF + svg2pdf.js installed, all TypeScript compiles cleanly</done>
</task>

<task type="auto">
  <name>Task 3: Create geometry computation modules (dimension layout, chamber, assembly, title block)</name>
  <files>
    frontend/src/lib/drawings/dimension-layout.ts
    frontend/src/lib/drawings/chamber-geometry.ts
    frontend/src/lib/drawings/assembly-geometry.ts
  ...
  <action>
1. **frontend/src/lib/drawings/dimension-layout.ts** -- Smart stagger algorithm:
...
2. **frontend/src/lib/drawings/chamber-geometry.ts** -- Chamber clearance computation:
...
3. **frontend/src/lib/drawings/assembly-geometry.ts** -- Assembly composition:
...
4. **frontend/src/lib/drawings/title-block.ts** -- Title block data:
  ...
  <done>4 files exist in frontend/src/lib/drawings/ (dimension-layout, chamber-geometry, assembly-geometry, title-block), all TypeScript compiles cleanly, all functions are pure (no React imports)</done>
</task>
...
6. `grep -rL "from 'react'" frontend/src/lib/drawings/` -- all 8 files returned (none import React)
</verification>
...
- Rifle create/edit schemas accept the 3 optional nullable chamber fields
